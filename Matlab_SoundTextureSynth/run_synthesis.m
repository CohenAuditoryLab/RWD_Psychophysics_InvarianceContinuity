...
if P.imposition_windowing==2 %imposition is not circular, so leave off ends where window faded to zero
    audiowrite([P.output_folder new_filename num2str(iteration_count) '.wav'], synth_sound(P.audio_sr:end-P.audio_sr), sr);
else
    audiowrite([P.output_folder new_filename num2str(iteration_count) '.wav'], synth_sound, P.audio_sr);
    if P.write_repeat
        audiowrite( [P.output_folder new_filename num2str(iteration_count) '_rep.wav'], [synth_sound;synth_sound], P.audio_sr);
    end
end
