...
SFA_scores_batch = np.zeros([total_stimuli,5,8])
Baseline_scores_batch = np.zeros([total_stimuli,5,8])
SFA_scores_batch_null = np.zeros([total_stimuli,5,8])
Baseline_scores_batch_null = np.zeros([total_stimuli,5,8])
...
for cur_pair in stimuli_ind_list:  #run a quick batch here to check that is works then run on kilosort.  Works so lets port it over to kilosort
    ...
    try:
        ...
        [SFA_scores_batch[cur_pair,:,:] , Baseline_scores_batch[cur_pair,:,:] ]= rsfa.run_SFA(cur_pair,vocal_foldername,all_pairs[cur_pair],clutterfoldername, clutterfiletemplate, run_nulls)
    except:
...
for cur_pair in stimuli_ind_list:  #run a quick batch here to check that is works then run on kilosort.  Works so lets port it over to kilosort
    ...
    try:
        ...
        [SFA_scores_batch_null[cur_pair,:,:] , Baseline_scores_batch_null[cur_pair,:,:] ]= rsfa.run_SFA(cur_pair,vocal_foldername,all_pairs[cur_pair],clutterfoldername, clutterfiletemplate, run_nulls)
    except:
...
#Need to fix this or set up  a separate plotting function/fix whatever separate plotting function we have.
...
#     np.shape(call_snr_avg[:,temp.astype('bool')]) = np.shape(call_snr_avg[:,temp.astype('bool')])  + score_avg[pair,:]
    
    
# plt.figure()
# plt.imshow(combo_counter, extent=[0,1,0,1])
# plt.xticks([0.125, 0.375, 0.625, 0.875], prefixes)
# plt.yticks([0.125, 0.375, 0.625, 0.875], prefixes)
