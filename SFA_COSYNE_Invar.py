...
NOTE THIS IS THE FILE I AM NOW USING FOR THE STIMULI FOR COSYNE 2019-10-17

note to self 2019-10-29 - set this up so parameters (e.g. number of features used and presentations) match clutter code.  Figure out by tonight then just solidify.
Only going to test number of vocalizations up to 3 for now since that is what we have time for.

Also need to fully think of x axis for this but probably going to average distance across a bunch of examples that fall into four catagories


Update: 2020-01-18 Testing out performance with monkey calls.  Going to just test some interesting natural questions first then
 will move on to trying out clutter stimuli with monkey calls.  It would be interesting to potential try with another transformation since AM doesn't seem to be viable
 Potentially could use reverb in this case?  Need to think on this further.
 
 Initial results recapitulated: 1) works on distinguishing monkey calls of different types by different monkeys (2 stimuli)
...
for a_stimulus in range(0,5):
    ...
    for a_round in range(0,10):
        ...
        vocal_files_train = ['Monkey_Calls/AG493B.wav', 'Monkey_Calls/CO1Z67.wav', 'Monkey_Calls/CS1E54.wav'] #['Stimulus_Set_1/AM_Stimulus_2_1.5.wav', 'Stimulus_Set_1/AM_Stimulus_4_2.5.wav', 'Stimulus_Set_1/AM_Stimulus_5_3.wav',\
        ...
        vocal_files_test = ['Monkey_Calls/AG493B.wav', 'Monkey_Calls/CO1Z67.wav','Monkey_Calls/CS1E54.wav'] #['Stimulus_Set_1/AM_Stimulus_1_1.wav', list_of_names[a_stimulus] ]
