...
%modified for updates in MATLAB 2018B by Ron DiTullio
%also commented more aggressively

    clear; clf
    infile='test1.wav';
    outfile='test1Changed.wav';
    ...
    [y,sr]=audioread(infile);                  % read song file, changed 2019-06-18, sr stands for sampling rate (i.e. Fs)
    %siz=audioread(infile,'size');             % length of song in samples, changed 2019-06-18, 
    siz = size(y); %actually wants size it looks like
    ...
    audiowrite(outfile,tt,sr,'BitsPerSample', 24); %changed 2019-06-18 now use audiowrite just like use audioread, also made 24 bits
    fclose('all');